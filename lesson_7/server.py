...
from common.variables import action, acc_name, response, max_connect, \
    presence, time_var, user, error, port, ip_address, sender, MESSAGE, MESSAGE_TEXT
...
def process_client_message(message, messages_list, client):
    ...
    if action in message and message[action] == presence and time_var in message \
        ...
        return
            ...
            time_var in message and MESSAGE_TEXT in message:
        messages_list.append((message[acc_name], message[MESSAGE_TEXT]))
        return
    else:
        send_message(client, {
            response: 400,
